...
esp_err_t ds1307_read_time(i2c_master_dev_handle_t rtc_handle, ds1307_datetime_t *dt)
{
    ...
    dt->sec = bcd_to_dec(data[0] & 0x7F);
    dt->min = bcd_to_dec(data[1] & 0x7F);
    // 24-hour mode
...
esp_err_t ds1307_set_time(i2c_master_dev_handle_t rtc_handle, const ds1307_datetime_t *dt)
{
    ...
    // seconds register: bit7 CH = 0, oscillator enabled
    data[1] = dec_to_bcd(dt->sec) & 0x7F;
    data[2] = dec_to_bcd(dt->min);
    data[3] = dec_to_bcd(dt->hour); // 24-hour mode
    ...
    return i2c_master_transmit(rtc_handle, data, sizeof(data), -1);
}

esp_err_t ds1307_debug_dump(i2c_master_dev_handle_t rtc_handle)
{
    uint8_t reg = 0x00;
    uint8_t data[7] = {0};
